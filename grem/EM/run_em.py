...
def run_em_def(conf_file = "../conf/minimal-em-configuration.json", sr_pop_name = "all", project_dir_in_file = ""):
    ...
    config = {
        "imputation_input_file": project_dir_in_file + json_conf.get("imputation_in_file"), #"/".join(json_conf.get("imputation_in_file").strip("/").split('/')[1:]) ,
    ...
    while not_converge and iteration < config["max_iteration"]:
        ...
        runfile_em_mt.run(plan_b=planB, config_file=conf_file, pop=[pop], count_by_prob=np.array([1]), num_subjects = num_saples, project_dir_in_file = project_dir_in_file )
        last_logl = logL
    ...
    if not config["just_SR"]:
        ...
        while not_converge and iteration < config["max_iteration"]:
            ...
            runfile_em_mt.run(planB, conf_file,count_by_prob = count_by_prob, em_mr =True, iteration =iteration, num_subjects = num_saples, project_dir_in_file = project_dir_in_file )
            last_logl = logL
